...
class message_label extends rcube_plugin {
    ...
    function startup($args) {
        ...
        if ($search == 'labelsearch') {
            ...
            if ($args['action'] == 'compose') {
                $draft_uid = rcube_utils::get_input_value('_draft_uid', RCUBE_INPUT_GET);
                if (!empty($draft_uid)) {
            ...
            if ($args['action'] == 'mark') {
                $flag = rcube_utils::get_input_value('_flag', RCUBE_INPUT_POST);
                $uid = rcube_utils::get_input_value('_uid', RCUBE_INPUT_POST);

                $post_str = '_flag=' . $flag . '&_uid=' . $uid;
                if ($quiet = rcube_utils::get_input_value('_quiet', RCUBE_INPUT_POST))
                    $post_str .= '&_quiet=' . $quiet;
                if ($from = rcube_utils::get_input_value('_from', RCUBE_INPUT_POST))
                    $post_str .= '&_from=' . $from;
                if ($count = rcube_utils::get_input_value('_count', RCUBE_INPUT_POST))
                    $post_str .= '&_count=' . $count;
                if ($ruid = rcube_utils::get_input_value('_ruid', RCUBE_INPUT_POST))
                    $post_str .= '&_ruid=' . $ruid;
            ...
            if ($args['action'] == 'moveto') {
                $target_mbox = rcube_utils::get_input_value('_target_mbox', RCUBE_INPUT_POST);
                $uid = rcube_utils::get_input_value('_uid', RCUBE_INPUT_POST);
    ...
    function message_label_imap_set() {
        if (($uids = rcube_utils::get_input_value('_uid', RCUBE_INPUT_POST)) && ($flag = rcube_utils::get_input_value('_flag', RCUBE_INPUT_POST))) {
            $flag = $a_flags_map[$flag] ? $a_flags_map[$flag] : strtoupper($flag);
            $type = rcube_utils::get_input_value('_type', RCUBE_INPUT_POST);
            $label_search = rcube_utils::get_input_value('_label_search', RCUBE_INPUT_POST);
    ...
    function message_label_search() {
        ...
        $_SESSION['page'] = 1;
        $page = rcube_utils::get_input_value('_page', RCUBE_INPUT_POST);
        ...
        if ($sort = rcube_utils::get_input_value('_sort', RCUBE_INPUT_POST)) {
    ...
    function message_label_mark() {
        ...
        if (($uids = rcube_utils::get_input_value('_uid', RCUBE_INPUT_POST)) && ($flag = rcube_utils::get_input_value('_flag', RCUBE_INPUT_POST))) {
            ...
            if ($flag == 'DELETED' && $read_when_deleted && !empty($_POST['_ruid'])) {
                $uids = rcube_utils::get_input_value('_ruid', RCUBE_INPUT_POST);
                $uids = explode(',', $uids);
    ...
    function message_label_move() {
        if (!empty($_POST['_uid']) && !empty($_POST['_target_mbox'])) {
            ...
            $target = rcube_utils::get_input_value('_target_mbox', RCUBE_INPUT_POST);
            $uids = explode(',', $uids);
    ...
    function display_list_preferences() {
        $display_label = rcube_utils::get_input_value('_display_label', RCUBE_INPUT_POST);
        $display_folder = rcube_utils::get_input_value('_display_folder', RCUBE_INPUT_POST);
    ...
    function label_save($args) {
        ...
        $id = rcube_utils::get_input_value('_label_id', RCUBE_INPUT_POST);
        $header = rcube_utils::get_input_value('_label_header', RCUBE_INPUT_POST);
        $folder = rcube_utils::get_input_value('_folder_search', RCUBE_INPUT_POST);
        $input = rcube_utils::get_input_value('_label_input', RCUBE_INPUT_POST);
        $color = rcube_utils::get_input_value('_label_color', RCUBE_INPUT_POST);
        $text = rcube_utils::get_input_value('_label_text', RCUBE_INPUT_POST);
    ...
    function action_check_mode() {
        $check = rcube_utils::get_input_value('_check', RCUBE_INPUT_POST);
        $this->rc = rcmail::get_instance();
